package com.example.pinterestdownloaderci.work
...
class DownloadWorker(appContext: Context, params: WorkerParameters): CoroutineWorker(appContext, params) {
    ...
    override suspend fun doWork(): Result {
        try {
            ...
            //Se obtiene la ruta del directorio especificado por el usuario y se le agrega a la ruta la carpeta 'Pinterest Downloader CI'
            ...
            val notification = NotificationCompat.Builder(applicationContext, "pinterest_download")
                .setPriority(NotificationCompat.PRIORITY_MAX)
                .setSmallIcon(R.drawable.ic_stat_pdci)
            ...
            if(downloadMode == "audio"){
                ...
                request.addOption("-o", directory.absolutePath + "/pinterest@%(uploader)s_%(display_id)s_audio_${formatConverted}.%(ext)s")
            }

            //Se verifica si la descarga es de tipo 'video'
            if(downloadMode == "video"){
                //Se añade el formato que tendrá el nombre del archivo y la ubicación donde se descargará el archivo
                request.addOption("-o", directory.absolutePath + "/pinterest@%(uploader)s_%(display_id)s.%(ext)s")
            }
            ...
            val downloadReq = YoutubeDL.getInstance().execute(request, taskId) {
                ...
                val notificationDownload3 =
                    NotificationCompat.Builder(applicationContext, "pinterest_download")
                        .setPriority(NotificationCompat.PRIORITY_MAX)
                        .setSmallIcon(R.drawable.ic_stat_pdci)
                        .setContentTitle(name)
                        .setStyle(
...
//Función para crear el canal de notificaciones, necesario a partir de Android Oreo
    private fun createNotificationChannel(){
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
            val notificationChannel = notificationManager?.getNotificationChannel("pinterest_download")
            if(notificationChannel == null){
                ...
                val channel = NotificationChannel("pinterest_download", name, importance).apply {
